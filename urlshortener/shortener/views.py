...
def get_url(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponse(
                "Voici le l allias pour le lien : "
                + request.POST.get("your_url", "") +
                '<br /><script src="//cdnjs.cloudflare.com/ajax/libs/'
                'clipboard.js/1.4.0/clipboard.min.js"></script>' +
                '<script type="text/javascript">(function(){new Clipboard'
                '("#copy-button");})();</script>' +
                '<input id="post-shortlink" value="localhost:8000/' +
                URL.get_or_create(request.POST.get("your_url", "")).alias +
                '/"><button class="button" id="copy-button" '
                'data-clipboard-target="#post-shortlink">Copy</button>',
                status=201)
        else:
            return render(
                request,
